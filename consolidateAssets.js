function consolidateAssets() {
    chrome.storage.local.get("parsedAssets", function(result) {
        ...
        for (const category in categoryTotals) {
            ...
            results.push(`${category}: ${categoryValue.toLocaleString()} € (${percentage}%)`);
...
function createPieChart(data, container) {
    ...
    data.forEach(item => {
        ...
        // Ajout du texte
        if (percentage > 0.05) { // N'afficher le texte que si le segment est assez grand (>5%)
...
function showModal(content, chartData, totalValue) {
    ...
    closeButton.onmouseover = () => {
        closeButton.style.background = "linear-gradient(to bottom, #f4b800, #cc9900)";
