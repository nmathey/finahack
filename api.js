export class FinaryClient {
    ...
    async requestNewToken(retryCount = 0) {
        ...
        console.log(`🔄 Tentative ${retryCount + 1}/${MAX_RETRIES + 1} de renouvellement du token...`);
        ...
        return new Promise((resolve, reject) => {
            chrome.tabs.query({active: true, currentWindow: true}, async function(tabs) {
                ...
                chrome.tabs.sendMessage(tabs[0].id, { action: "REQUEST_TOKEN" }, async (response) => {
                    ...
                    if (response?.token) {
                        await chrome.storage.local.set({ sessionToken: response.token });
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 500 && retryCount < MAX_RETRIES) {
                    retryCount++;
                    console.log(`🔄 Retry ${retryCount}/${MAX_RETRIES} after 500 error`);
