...
function flattenAssets(apiResponse) {
    ...
    apiResponse.result.forEach(holding => {
        ...
        const baseInfo = {
            ...
            institutionName: holding.institution?.name || holding.bank?.name || "Autre",
            envelopeType: envelopeType
        };

        // Helper pour ajouter un actif uniformisé
        const pushAsset = (id, name, assetType, category, subcategory, value, quantity, pnl) => {
        ...
        if (holding.cryptos) {
            holding.cryptos.forEach(c => {
                ...
                pushAsset(c.id, name, 'crypto', category, subcategory, c.display_current_value, c.quantity, c.display_unrealized_pnl);
            });
        }
        ...
        if (holding.securities) {
            holding.securities.forEach(s => {
                ...
                pushAsset(s.id, s.security.name, 'security', isEtf ? 'fund' : 'stock', s.security.security_type, s.display_current_value, s.quantity, s.display_unrealized_pnl);
            });
        }
        ...
        if (holding.real_estates) {
            holding.real_estates.forEach(re => {
                pushAsset(re.id, re.address_formatted || holding.name, 'real_estate', 'real_estate', 'physical', re.display_current_value, 1, re.display_unrealized_pnl);
            });
        }
        ...
        if (holding.scpis) {
            holding.scpis.forEach(s => {
                pushAsset(s.id, s.scpi.name, 'scpi', 'real_estate', 'paper', s.display_current_value, s.shares, s.display_unrealized_pnl);
            });
        }
        ...
        if (holding.fiats) {
            holding.fiats.forEach(f => {
                pushAsset(f.id, holding.name, 'fiat', 'fiat', 'cash', f.display_current_value, f.quantity, 0);
            });
        }
        ...
        if (holding.fonds_euro) {
            holding.fonds_euro.forEach(fe => {
                pushAsset(fe.id, fe.fund?.name || fe.name || "Fonds Euro", 'fonds_euro', 'fund', 'guaranteed', fe.display_current_value, fe.quantity, fe.display_unrealized_pnl);
            });
        }
        ...
        if (holding.startups) {
             holding.startups.forEach(s => {
                pushAsset(s.id, s.startup.name, 'startup', 'startup', 'private_equity', s.display_current_value, s.shares, s.display_unrealized_pnl);
            });
        }
    });
